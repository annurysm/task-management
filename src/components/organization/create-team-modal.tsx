...
interface CreateTeamModalProps {
  ...
  onClose: () => void
  onSubmit: (organizationId: string, name: string, description: string) => Promise<void>
}

export function CreateTeamModal({ organizationId, onClose, onSubmit }: CreateTeamModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await onSubmit(organizationId, name.trim(), description.trim())
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-zinc-900 rounded-lg shadow-xl max-w-md w-full mx-4">
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-zinc-700">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-zinc-100">Create Team</h2>
          <button
            onClick={onClose}
